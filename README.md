...
- **Anti-detection/evasion layer:** this means anomaly-aware pacing, duplicate suppression, content diversity, session stability, rate-limit compliance, and automatic cooldown. Per canonical `schemas/policy-contract.json` (v1.0.0, `maxAutonomousRisk: High`), CAPTCHAs on `Low`/`Moderate`/`High` per-action are solved via the audited `auto_ensemble` (`capsolver` → `2captcha` → `ai-captcha-bypass` → `buster`); `Very High`/`Critical` go to `auto_quarantine`. It must not circumvent bans/suspensions by rotating identity/IP or submit through explicitly blocked endpoints after a platform block.
...
The default and maximum production autonomous threshold is **High** per canonical `schemas/policy-contract.json` (v1.0.0); it may only be tightened to `Moderate`/`Low`, not raised above `High`. A requested action whose best supported route is `Very High` or `Critical` is returned as `auto_quarantine`; `High` executes as `auto_with_verification` — this is independent of the old coarse platform-level risk. Thus LinkedIn `post` executes as `Low` through `official_api`, while LinkedIn `dm` is quarantined as `Critical`. Product Hunt browse/data is Low, owned submit is Moderate, Quora post is High (executable with verification), and vote automation is Critical/quarantined.
