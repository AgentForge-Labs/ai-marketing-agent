...
- A persona never grants permission to bypass a ban/suspension by rotating identity/IP. Per canonical `schemas/policy-contract.json` (v1.0.0, `maxAutonomousRisk: High`), CAPTCHA/security challenges on `Low`/`Moderate`/`High` per-action are solved via the audited `auto_ensemble`; `Very High`/`Critical` go to `auto_quarantine`. Reopening a banned/suspended account — even with a fresh profile AND fresh IP — is forbidden (quarantine/appeal only); a fresh pair is allowed only when no ban exists AND the platform explicitly permits multi-account (`accountReuse`, enforced by `assert_reopen_allowed`).
